...
class HomeViewModel : ViewModel() {
    ...
    fun loadAndCancel(
        ...
        ioSafe {
            ...
            val validSources = context?.filterProviderByPreferredMedia()?.filter { it.hasMainPage && it.name != noneApi.name }
            if (preferredApiName == noneApi.name || preferredApiName.isNullOrBlank()) {
                if (!validSources.isNullOrEmpty()) {
                    val autoApi = validSources.first()
                    DataStoreHelper.currentHomePage = autoApi.name
                    loadAndCancel(autoApi)
                } else {
                ...
                if (validSources.isNullOrEmpty()) {
                    ...
                    val apiRandom = validSources.random()
