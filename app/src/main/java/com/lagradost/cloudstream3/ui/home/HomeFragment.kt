...
class HomeFragment : BaseFragment<FragmentHomeBinding>(
    ...
    companion object {
        ...
        fun Context.selectHomepage(selectedApiName: String?, callback: (String) -> Unit) {
            ...
            fixSystemBarsPadding(binding.root, padTop = false, padBottom = true)
            builder.show()
            builder.let { dialog ->
    ...
    override fun onBindingCreated(binding: FragmentHomeBinding) {
        com.lagradost.cloudstream3.analytics.AnalyticsManager.logScreen("Home")
        context?.let { HomeChildItemAdapter.updatePosterSize(it) }
        activity?.let { act ->
            com.lagradost.cloudstream3.updates.UpdateManager.checkForUpdateOnStartup(act)
        }
